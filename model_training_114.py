...
Veri      : features.csv  (114 feature, 52.830 örnek)
Split     : %80 train / %20 test  (stratify, random_state=42)
           'split' kolonu varsa kullanılır, yoksa otomatik oluşturulur
...
print('=' * 62)
print('  JS Kötücül Kod Tespiti — 12 Model (114 Feature)')
print('=' * 62)
...
if 'split' in df.columns:
    ...
    print(f'\n  Split kolonu bulundu — kullanılıyor.')
else:
...
sample_w  = np.where(y_train == 0, cw[0], cw[1])   # KNN için
...
# [2] Naive Bayes — class_weight desteklemiyor, varsayılan prior kullanılır
...
# [4] SVM — büyük datasette yavaş olabilir
...
# [7] XGBoost — isim çakışmasını önlemek için xgb_model
print('[7/12] XGBoost...')
t         = time.time()
...
df_res.to_csv(f'{RESULTS}/model_sonuclari.csv', index=False)
...
plt.suptitle('12 Model — Tüm Metrikler  (114 Ham Feature)',
...
ax.set_title('Model Karşılaştırması — F1 Skoru\n(Mavi: ML  |  Kırmızı: DL)  [114 Feature]',
             fontsize=13, fontweight='bold')
ax.axvline(x=0.95, color='orange', linestyle='--', alpha=0.5, label='0.95 eşiği')
...
for ax, grup, baslik in zip(
    ...
    ax.spines[['top', 'right']].set_visible(False)
plt.suptitle('ROC Eğrileri — 114 Ham Feature', fontsize=14, fontweight='bold')
plt.tight_layout()
...
plt.suptitle('En İyi 4 Model — Confusion Matrix  [114 Feature]',
             fontsize=14, fontweight='bold')
plt.tight_layout()
...
ax.set_title('Geleneksel ML vs Derin Öğrenme — Ortalama Performans  [114 Feature]',
...
ax.set_title('Model Eğitim Süreleri  [114 Feature]', fontsize=13, fontweight='bold')
ax.legend(handles=[mpatches.Patch(color='#2980b9', label='Geleneksel ML'),
...
print(f"""
{'='*62}
  ✅ MODEL EĞİTİMİ TAMAMLANDI  (114 Feature)
{'='*62}
